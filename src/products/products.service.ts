...
export class ProductsService {
  ...
  async calculateLiquidityScore(data: {
    averageRating: number;
    reviewCount: number;
    price: number;
  }): Promise<{ score: number; status: string }> {
    const { averageRating, reviewCount, price } = data;
    ...
    const normalizedRating = Math.min(averageRating / 5, 1);
    const normalizedReviews = Math.min(reviewCount / 1000, 1);
    const normalizedPrice = price > 0 ? 1 - Math.min(price / 1000, 1) : 0;
